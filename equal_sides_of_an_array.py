...
def find_even_index(arr):
	...
	for idx in range(len(arr)):
		if idx == 0:
			left = 0 
			right = reduce(lambda x, y: x+y, arr[idx+1:])
			if left == right:
				return idx
		else:
			left = reduce(lambda x, y: x+y, arr[0:idx]) if idx > 1 else arr[0]
			right = reduce(lambda x, y: x+y, arr[idx+1:]) if idx+1 < len(arr) else 0
			if left == right:
				return idx
	else:
		return -1
