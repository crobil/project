...
def pick_peaks(arr):
	...
	for idx in range(1, len(arr)-1):
		if arr[idx] > arr[idx-1]:
			for sub_idx in range(idx+1, len(arr)):
				if arr[sub_idx] > arr[idx]:
					break
				elif arr[sub_idx] < arr[idx]:
					_dict['pos'].append(idx)
					_dict['peaks'].append(arr[idx])
					break
...
print pick_peaks([-4, 7, 11, 3, 1, -1, -4, 13, 2, 13, 16, -5, 10, 15, 9, -5, -2, 16, 4, -1, 3, 9, 18, 8, 15])
